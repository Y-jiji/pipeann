  ...
  void SSDIndex<T, TagT>::pipe_search_common(const T *query1, uint32_t mem_L, uint64_t l_search, uint64_t l_pool,
    ...
    auto add_to_retset = [&](const unsigned *node_ids, const uint64_t n_ids, float *dists) {
      for (uint64_t i = 0; i < n_ids; ++i) {
        PTRACE_IN("S0 %lu %u %.9g", (unsigned long) cur_list_size, node_ids[i], (double) dists[i]);
    ...
    if (mem_L) {
      ...
      PTRACE_IN("S0 0 %u %.9g", meta_.entry_point_id, (double) dist_scratch[0]);
    ...
    auto poll_all = [&]() -> std::pair<int, int> {
      ...
      while (!on_flight_ios.empty() && on_flight_ios.front().finished()) {
        ...
        PTRACE_REF("C %u %d", io.nbr.id, (int) in_pool);
        in_pool ? ++n_in : ++n_out;
        this->unlock_idx(idx_lock_table, io.nbr.id);
        on_flight_ios.pop();
      }
      PTRACE_REF("P %u %u", n_in, n_out);
    ...
    auto calc_best_node = [&]() -> int {
      ...
      for (marker = 0; marker < cur_list_size; ++marker) {
        if (!retset[marker].visited && id_buf_map.find(retset[marker].id) != id_buf_map.end()) {
          PTRACE_IN("X %u %u", retset[marker].id, marker);
          retset[marker].flag = false;
    ...
    auto terminate = [&]() -> bool {
      ...
      if (stop) {
        PTRACE_REF("T %lu %d %d", (unsigned long) is_member_cnt, ret, (int) range_crossed);
      }
      return stop;
    };
    ...
    PTRACE_IN("Q %lu %lu %lu %u %lu", (unsigned long) l_search, (unsigned long) l_pool, (unsigned long) beam_width, mem_L,
    ...
    if (mem_L) {  // overlap init with the first read.
      ...
      for (unsigned i = 0; i < cur_list_size; ++i) {
        PTRACE_IN("S1 %u %u %.9g", i, retset[i].id, (double) retset[i].distance);
      }
    }
    ...
    while (!terminate()) {
      ...
      PTRACE_REF("B %ld %u %u", (long) cur_beam_width, marker, max_marker);
    }
